...
import { createReportTemplateV2Container } from './report-template-v2.container'
import {
  ReportTemplateV2Service,
  type ReportTemplateV2ServiceError,
} from './report-template-v2.service'
import {
...
function template(
  overrides: Partial<ReportTemplateV2['metadata']> = {},
): ReportTemplateV2 {
  const field = {
    name: 'responsavel',
    label: 'Responsável',
...
function repositoryMock(): ReportTemplateV2Repository {
  return {
...
describe('ReportTemplateV2Service', () => {
  let repository: ReportTemplateV2Repository
  let service: ReportTemplateV2Service

  beforeEach(() => {
    repository = repositoryMock()
    service = new ReportTemplateV2Service(repository, () => SERVICE_TIME)
  })
  ...
  it('delega getById e retorna uma cópia independente', async () => {
    ...
    const result = await service.getById('template-v2')
    expect(repository.getById).toHaveBeenCalledWith('template-v2')
  ...
  it('rejeita update quando o template não existe', async () => {
    await expect(service.update(template())).rejects.toMatchObject({
      code: 'NOT_FOUND',
    } satisfies Partial<ReportTemplateV2ServiceError>)
    expect(repository.update).not.toHaveBeenCalled()
  })

  it('delega delete sem introduzir semântica adicional', async () => {
    await service.delete('template-v2')
    expect(repository.delete).toHaveBeenCalledWith('template-v2')
  })

  it('confirma um template draft e persiste a transição', async () => {
    const stored = template({ status: 'draft' })
    vi.mocked(repository.getById).mockResolvedValue(stored)
    const confirmed = await service.confirm('template-v2')
    expect(repository.update).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('rejeita versão inválida antes de criar, atualizar ou confirmar', async () => {
    const invalid = { ...template(), version: 1 } as unknown as ReportTemplateV2
    ...
    await expect(service.confirm('template-v2')).rejects.toMatchObject({
      code: 'INVALID_VERSION',
    })
  })
  ...
  it('não modifica silenciosamente os objetos recebidos ou persistidos', async () => {
    ...
    vi.mocked(repository.getById).mockResolvedValue(stored)
    await service.confirm('template-v2')
    expect(stored).toEqual(storedSnapshot)
  })

  it('compõe instâncias V2 independentes sem criar singleton global', () => {
    const first = createReportTemplateV2Container()
    const second = createReportTemplateV2Container()
    expect(first.service).toBeInstanceOf(ReportTemplateV2Service)
