...
import {
  createReportTemplateV2CreationContainer,
  createReportTemplateV2Container,
} from './report-template-v2.container'
import { ReportTemplateV2CreationService } from './report-template-v2-creation.service'
import {
...
function richTemplate(): ReportTemplateV2 {
  const fieldEvidence = {
...
describe('ReportTemplateV2CreationService', () => {
  it('encadeia pipeline e service preservando progresso e o template analisado', async () => {
    ...
    const applicationService = new ReportTemplateV2CreationService(
      pipeline,
      templateService,
    )
    ...
    expect(pipeline.executeV2).toHaveBeenCalledWith('modelo.docx', progress)
    expect(templateService.create).toHaveBeenCalledWith(analyzed)
    expect(result.metadata.status).toBe('draft')
  })

  it('registra e recupera integralmente o template no repository V2', async () => {
    const analyzed = richTemplate()
    const pipeline = { executeV2: vi.fn().mockResolvedValue(analyzed) }
    const container = createReportTemplateV2CreationContainer(pipeline)

    const result = await container.creationService.createFromDocument(
      'modelo.docx',
    )
