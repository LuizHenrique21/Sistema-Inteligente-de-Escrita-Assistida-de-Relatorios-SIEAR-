...
import {
  InMemoryReportTemplateV2Repository,
  type ReportTemplateV2RepositoryError,
} from './in-memory-report-template-v2.repository'
...
function richTemplate(id = 'template-v2'): ReportTemplateV2 {
  const field = {
    name: 'responsavel',
    label: 'Responsável',
    type: 'text' as const,
...
describe('InMemoryReportTemplateV2Repository', () => {
  let repository: InMemoryReportTemplateV2Repository

  beforeEach(() => {
    repository = new InMemoryReportTemplateV2Repository()
  })

  it('cria e recupera um template V2 pelo ID', async () => {
    const created = await repository.create(richTemplate())
    expect(created.metadata.id).toBe('template-v2')
    await expect(repository.getById('template-v2')).resolves.toEqual(created)
  })
  ...
  it('atualiza somente um template existente', async () => {
    ...
    expect((await repository.getById('template-v2'))?.metadata.updatedAt).toBe(
  ...
  it('rejeita ID duplicado sem sobrescrever o existente', async () => {
    ...
    await expect(repository.create(duplicate)).rejects.toMatchObject({
      code: 'DUPLICATE_ID',
    } satisfies Partial<ReportTemplateV2RepositoryError>)
    expect((await repository.getById('template-v2'))?.metadata.name).toBe(
      'Modelo técnico V2',
    )
  })
  ...
  it('retorna null ao buscar e rejeita atualização de template inexistente', async () => {
    ...
    await expect(repository.update(richTemplate('missing'))).rejects.toMatchObject(
      {
        code: 'NOT_FOUND',
      } satisfies Partial<ReportTemplateV2RepositoryError>,
    )
  ...
  it('rejeita versão incorreta e objetos V1 de maneira controlada', async () => {
    const invalidVersion = {
      ...richTemplate(),
      version: 1,
    } as unknown as ReportTemplateV2
    await expect(repository.create(invalidVersion)).rejects.toMatchObject({
      code: 'VALIDATION_ERROR',
      message: expect.stringContaining(String(REPORT_TEMPLATE_V2_VERSION)),
    })
    await expect(
      repository.create({ id: 'legacy-v1' } as unknown as ReportTemplateV2),
  ...
  it('isola o armazenamento de mutações no objeto original e retornado', async () => {
    ...
    const firstRead = await repository.getById('template-v2')
    ...
    const secondRead = await repository.getById('template-v2')
    expect(
    ...
    expect(
      (await repository.getById('template-v2'))?.formattingPattern
        .headingStyles[0]?.formatting.fontSizePt,
    ).toBe(13)
  })
